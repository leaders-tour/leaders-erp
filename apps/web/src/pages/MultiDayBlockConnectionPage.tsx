...
function BlockConnectionEditShell({
  listModal,
  onClose,
  children,
}: {
      ...
      >
        ...
        >
          <div className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
            <h2 className="text-lg font-semibold text-slate-900">블록 후속 연결 수정</h2>
...
export function MultiDayBlockConnectionPage({ mode = 'all' }: MultiDayBlockConnectionPageProps): JSX.Element {
  ...
  const showEditSection = isEditMode || (mode === 'list' && editingId !== null);
  const pageTitle =
    mode === 'create'
      ? '블록 후속 연결 생성'
      : mode === 'list'
  ...
  return (
    <section className="grid gap-6">
      ...
      {showEditSection ? (
        <BlockConnectionEditShell listModal={mode === 'list' && Boolean(editingId)} onClose={() => resetForm()}>
          <form
            className="grid gap-4"
            onSubmit={async (event) => {
              event.preventDefault();
              ...
              <div className="grid gap-6">
                ...
                <div className="grid gap-3 rounded-2xl border border-slate-200 p-4">
                  <label className="grid gap-1 text-sm min-w-0">
                    ...
                    <div className="relative">
                      ...
                      />
                      {blockOpen ? (
                        <div className="absolute left-0 right-0 top-[44px] z-[100] max-h-56 overflow-auto rounded-xl border border-slate-200 bg-white p-1 shadow-lg">
                          {filteredBlockOptions.length === 0 ? (
                            <div className="px-3 py-2 text-sm text-slate-500">검색 결과가 없습니다.</div>
                          ) : (
                            filteredBlockOptions.map((stay) => (
                ...
                <div>
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      if (mode === 'list' && editingId) {
                        resetForm();
                        return;
                      }
                      navigate(editingId ? `/multi-day-blocks/connections/${editingId}` : '/multi-day-blocks/connections/list');
                    }}
                    className="ml-2"
                  >
                    취소
