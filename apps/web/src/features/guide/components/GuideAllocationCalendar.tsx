...
import type { GuideWithTrips, TripSlim } from '../hooks';
import { getColorByDestination } from '../trip-color';

interface Props {
  guides: GuideWithTrips[];
  year: number;
...
/** ISO 문자열("2026-06-08" 또는 "2026-06-08T…") → 로컬 자정 Date */
...
function buildBlocks(trips: TripSlim[], year: number, month: number): TripBlock[] {
  ...
  for (const trip of trips) {
    ...
    // 시작일은 0-indexed day offset (monthStart=0)
...
export function GuideAllocationCalendar({ guides, year, month, onChangeMonth }: Props) {
  const navigate = useNavigate();
  const [tooltip, setTooltip] = useState<{ trip: TripSlim; guideId: string } | null>(null);
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="divide-y divide-slate-100">
        ...
        {guides.map((guide) => {
          const blocks = buildBlocks(guide.confirmedTrips, year, month);
          const hasTrips = blocks.length > 0;

          return (
            <div key={guide.id} className="flex min-h-[3.5rem]">
              {/* 좌측: 가이드 정보 */}
              <button
                type="button"
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="truncate text-xs font-semibold text-slate-800">{guide.nameKo}</p>
                  <span
                    className={`mt-0.5 inline-block rounded-full px-1.5 py-px text-[10px] font-medium ${
                      LEVEL_COLOR[guide.level] ?? LEVEL_COLOR.OTHER
                    }`}
                  >
              ...
              <div className="relative flex-1 overflow-x-auto">
                ...
                {todayOff !== null && (
                  <div
                    ...
                    style={{
                      left: `calc(${((todayOff + 0.5) / daysInMonth) * 100}% )`,
                    }}
                  />
                )}
                ...
                <div className="relative h-full px-0.5 py-2">
                  ...
                  {blocks.map(({ trip, leftPct, widthPct }) => {
                    ...
                    const s = getTripStart(trip);
                    const e = getTripEnd(trip);
                    const isHovered = tooltip?.trip.id === trip.id && tooltip?.guideId === guide.id;
                    ...
                    return (
                      ...
                      >
                        <button
                          ...
                          onMouseEnter={() => setTooltip({ trip, guideId: guide.id })}
