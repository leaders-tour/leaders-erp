...
type ConfirmationPdfJobStatus = 'queued' | 'running' | 'succeeded' | 'failed';

export type ConfirmationPdfDownloadPhase = 'idle' | 'queued' | 'rendering' | 'downloading';

interface ConfirmationPdfJobResponse {
  jobId: string;
  status: ConfirmationPdfJobStatus;
}

interface ConfirmationPdfJobStatusResponse extends ConfirmationPdfJobResponse {
  errorMessage?: string;
  filename: string;
  ready: boolean;
}

export interface ConfirmationPdfDownloadInput {
...
export function getConfirmationPdfDownloadLabel(phase: ConfirmationPdfDownloadPhase): string {
  switch (phase) {
    case 'queued':
      return 'PDF 생성 요청 중...';
    case 'rendering':
...
export function useConfirmationPdfDownload(): {
  ...
  const [phase, setPhase] = useState<ConfirmationPdfDownloadPhase>('idle');
  ...
  const pollConfirmationPdfJob = useCallback(
    async (jobId: string): Promise<ConfirmationPdfJobStatusResponse> => {
      ...
      while (Date.now() < deadline) {
        ...
        const payload = (await response.json()) as ConfirmationPdfJobStatusResponse;
  ...
  const downloadConfirmationPdf = useCallback(
    async (input: ConfirmationPdfDownloadInput): Promise<void> => {
      ...
      setPhase('queued');
      try {
        const response = await authorizedFetch(`${API_BASE_URL}/documents/confirmation/pdf-jobs`, {
          method: 'POST',
          headers: {
        ...
        const payload = (await response.json()) as ConfirmationPdfJobResponse;
        const job = await pollConfirmationPdfJob(payload.jobId);

        setPhase('downloading');
        const downloadResponse = await authorizedFetch(
          `${API_BASE_URL}/documents/confirmation/pdf-jobs/${encodeURIComponent(job.jobId)}/download`,
        );
